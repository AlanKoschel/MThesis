...
class NFOV():
    def __init__(self, height=800, width=800):
        self.FOV = [0.611, 0.611] # human eyese hor = 110° and vert = 70°
        self.PI = pi * 0.5   # +-90°
        self.height = height
        self.width = width
    ...
    def _calcSphericaltoGnomonic(self, convertedScreenCoord):
        ...
        if ENABLE_GRAPH_ANALYSIS:
            plt.figure("0: lat long coordinates")
            plt.plot(x, y, color = "blue")
            plt.plot(np.linspace(-pi/2, pi/2, len(x)), np.zeros(len(x)), color="black")
            plt.plot(np.zeros(len(y)), np.linspace(-pi/2, pi/2, len(x)), color="black")
            plt.plot(self.cp[0], self.cp[1], color="red", linewidth=5, marker="o")
            plt.savefig("C:\\1_GitAlan\\NFOV1.jpg")
        ...
        if ENABLE_GRAPH_ANALYSIS:
            ...
            plt.plot(lon, lat, color="blue")
            plt.plot(np.linspace(-pi/2, pi/2, len(x)), np.zeros(len(x)), color="black")
            plt.plot(np.zeros(len(y)), np.linspace(-pi/2, pi/2, len(x)), color="black")
            plt.plot(self.cp[0], self.cp[1], color="red", linewidth=5, marker="o")
            plt.savefig("C:\\1_GitAlan\\NFOV2.jpg")
        ...
        if ENABLE_GRAPH_ANALYSIS:
            ...
            plt.plot(lon, lat, color="blue")
            plt.plot(np.linspace(0, 1, len(x)), np.zeros(len(x)), color="black")
            plt.plot(np.zeros(len(y)), np.linspace(0, 1, len(x)), color="black")
            plt.plot((self.cp[0]/self.PI +1)*0.5, (self.cp[1]/self.PI +1)*0.5, color="red", linewidth=5, marker="o")
            plt.savefig("C:\\1_GitAlan\\NFOV3.jpg")
    ...
    def _bilinear_interpolation(self, screen_coord):
        ...
        if ENABLE_GRAPH_ANALYSIS:
            ...
            plt.plot(uf, vf, color="blue")
            plt.plot(np.linspace(0, self.frame_height, len(vf)), np.zeros(len(vf)), color="black")
            plt.plot(np.zeros(len(vf)), np.linspace(0, self.frame_width, len(vf)), color="black")
            plt.plot((self.cp[0]/self.PI +1)*0.5*3000, (self.cp[1]/self.PI +1)*0.5*3000, color="red", marker="o", linewidth=5)
            plt.savefig("C:\\1_GitAlan\\NFOV4.jpg")
